#!/usr/bin/env bash
# Pull LocalMD's on-device debug log (written by DebugLog.swift) off the
...
xcrun devicectl device copy from --device "$UDID" \
  ...
  --source Documents/localmd.log --destination "$DEST/localmd.log" --quiet
cat "$DEST/localmd.log"
